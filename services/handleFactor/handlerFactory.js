...
const deletePhotoFromCloud = (model) =>
  asyncHandler(async (req, res, next) => {
    ...
    } else if (req.body.storeLogoImage) {
      deleteImageFromCloudinary(document.storeLogoImagePublicId)
    } else if (req.body.storeCoverImage) {
      deleteImageFromCloudinary(document.storeCoverImagePublicId)
    }
    next()
  })
