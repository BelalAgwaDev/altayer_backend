...
//  @access Private/user
  ...
  exports.removeSpecificCartItem = asyncHandler(async (req, res, next) => {
    ...
    if (itemIndex === -1) {
      return next(new ApiError(`Item not found in the cart for this id: ${req.params.itemId}`, 404));
    }
    ...
    res.status(200).json({
      status: true,
      message: cart.cartItems.length === 0?"Successfully deleted logged user item from cart. The cart is now empty.":"Successfully deleted logged user item from cart",
